...
router.delete("/:id", auth, async (req, res) => {
  try {
    ...
    const teamMembers = await Employee.find({
      "reportingManager._id": req.params.id,
    });
    if (teamMembers.length > 0) {
      return res.status(400).json({
        message:
          "Cannot delete employee with team members. Please reassign team members first.",
      });
    }
  ...
  try {
    ...
    if (managerId) {
      ...
      if (managerId === req.params.id) {
        return res
          .status(400)
          .json({ message: "Employee cannot report to themselves" });
      }
